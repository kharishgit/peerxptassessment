...
def create_ticket(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            url = 'https://brototye.zendesk.com/api/v2/tickets.json'
            # user = 'harishvayyat@gmail.com'
...
def manage_tickets(request):
    ...
    if request.user.is_staff:
        ...
        email = request.user.email
        url = f'https://brototye.zendesk.com/api/v2/search.json?query=type:ticket requester:{email}'
        user = os.environ.get('USER_NAME')
        pwd = os.environ.get('PASSWORD')
        response = requests.get(url, auth=(user,pwd))
...
def delete_ticket(request, ticket_id):
    if request.user.is_staff:
        # Set the request parameters
        url = f'https://brototye.zendesk.com/api/v2/tickets/{ticket_id}.json'
        user = os.environ.get('USER_NAME')
        pwd = os.environ.get('PASSWORD')
